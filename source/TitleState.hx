...
class TitleState extends MusicBeatState
{
	...
	var ngSpr:FlxSprite;
	private var evilsk8rFix:FlxSprite;
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('giveALil'), 0);
				
				FlxG.sound.music.fadeIn(4, 0, 0.7);
				// ASSETS\PRELOAD\IMAGES\GFDANCETITLE.JSON!
			}
		}
	...
	function addMoreText2(text:String)
		{
			var coolText:Alphabet = new Alphabet(0, 0, text, true, false);
			coolText.screenCenter(X);
			coolText.y += (textGroup.length * 60) + 445;
			credGroup.add(coolText);
			textGroup.add(coolText);
		}
	
	function addMoreText3(text:String)
	{
		var coolText:Alphabet = new Alphabet(0, 0, text, true, false);
		coolText.screenCenter(X);
		coolText.y += (textGroup.length * 60) + 95;
		credGroup.add(coolText);
		textGroup.add(coolText);
	}
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 1:
				createCoolText(['ninjamuffin99', 'phantomArcade']);
			case 2:
				createCoolText(['ninjamuffin99', 'phantomArcade', 'kawaisprite', 'evilsk8r']);
			case 3:
				createCoolText(['ninjamuffin99', 'phantomArcade', 'kawaisprite', 'evilsk8r', 'present']);
			case 4:
				deleteCoolText();
			case 5:
				createCoolText2(['In association with']);
				addMoreText3('newgrounds');
			case 6:
				ngSpr.visible = true;
				addMoreText2('Shoutouts Tom Fulp');
			case 7:
				curWacky = FlxG.random.getObject(getIntroTextShit());
				deleteCoolText();
				ngSpr.visible = false;
			...
			case 9:
				deleteCoolText();
				ngSpr.visible = false;
				createCoolText(['refunk engine by']);
			case 10:
				deleteCoolText();
				createCoolText(['refunk engine by', 'kinsazo']);
			case 11:
				deleteCoolText();
				curWacky = FlxG.random.getObject(getIntroTextShit());
				createCoolText([curWacky[0]]);
